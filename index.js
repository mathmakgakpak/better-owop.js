...
class Client extends EventEmitter {
  ...
  static utils = {
    ...
    createChunkFromRGB(color) {
      let tile = new Uint8Array(16 * 16 * 3);
  ...
  constructor(options = {}) {
      ...
      */
      parseMessage(msg) {
        ...
        if (before.startsWith("[D]")) {
          ...
          user.rank = 0; //OJS.players[user.id] ? OJS.players[user.id].rank : 0;
          ...
          user.id = OJS.player.id;
          user.nick = OJS.player.nick;
          tell = true;
        }
    ...
    this.world = {
      ...
      move(x = that.player.x, y = that.player.y) {
        ...
        let dv = new WeirdDataViewSet(new ArrayBuffer(12));
        dv.setInt32(x);
        dv.setInt32(y);
      ...
      setPixel(x = that.player.x, y = that.player.y, color = that.player.color, wolfMove = true, sneaky, move = true) {
        ...
        if(wolfMove) {
          let distx = Math.trunc(x / Client.options.chunkSize) - Math.trunc(that.player.x / (Client.options.chunkSize * 16)); distx *= distx;
          let disty = Math.trunc(y / Client.options.chunkSize) - Math.trunc(that.player.y / (Client.options.chunkSize * 16)); disty *= disty;
        ...
        let dv = new WeirdDataViewSet(new ArrayBuffer(11));
        dv.setInt32(x);
        dv.setInt32(y);
        dv.setUint8(that.player.color[0]);
        dv.setUint8(that.player.color[1]);
      ...
      protectChunk(x, y, newState = 1) {
        ...
        let dv = new WeirdDataViewSet(new ArrayBuffer(10));
        dv.setInt32(x);
        dv.setInt32(y);
      ...
      clearChunk(x, y, color) {
        ...
        if(that.clientOptions.protocol === 0) {
          if(color[0] === 255 && color[1] === 255 && color[2] === 255) {
            let dv = new WeirdDataViewSet(new ArrayBuffer(9));
            dv.setInt32(x);
            dv.setInt32(y);
            that.ws.send(dv.buffer);
          } else {
          ...
          let dv = new WeirdDataViewSet(new ArrayBuffer(13));
          dv.setInt32(x);
        	dv.setInt32(y);
          dv.setUint8(color[0]);
          dv.setUint8(color[1]);
      ...
      __requestChunk(x, y) {
        let dv = new WeirdDataViewSet(new ArrayBuffer(8));
        dv.setInt32(x);
        dv.setInt32(y);
        that.ws.send(dv.buffer);
        return true;
        ...
        return new Promise(resolve => {
          OWOP.windowSys.addWindow(new OWOP.windowSys.class.window(`Verification Needed`, {
            ...
            grecaptcha.render(win.addObj(OWOP.util.mkHTML('div', {})), {
              theme: 'dark',
              sitekey: captchaSiteKey, // owop has 6LcgvScUAAAAAARUXtwrM8MP0A0N70z4DHNJh-KI
              callback: token => {
                win.close();
                resolve(token);
              }
  ...
  makeSocket() {
    ...
    ws.onmessage = e => {
      ...
      if(isBinary) {
        const dv = new WeirdDataViewGet(msg);
          ...
          case Client.options.opcode.captcha: {
            ...
            switch(id) {
              case Client.options.captchaState.WAITING: {
                ...
                } else if(this.clientOptions.captchaLoginToken) {
        ...
        let parsedMessage = Client.utils.parseMessage(msg);
...
let bot = new Client({ws:OWOP.options.serverAddress[0].url});
